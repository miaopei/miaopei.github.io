...
![03_C语言概述](program-c/03_C语言概述.png)

## 2. 数据类型

![04_数据类型](program-c/04_数据类型.png)
...
**`extern`告诉编译器这个变量或函数在其他文档里已被定义了。**
...
- `static`局部变量的生命周期和程序运行周期一样，同时staitc局部变量的值**只初始化一次，但可以赋值多次**
- `static`局部变量若未赋以初值，则由系统自动赋值：数值型变量自动赋初值0，字符型变量赋空字符
...
![fileinfo](program-c/fileinfo.png)
