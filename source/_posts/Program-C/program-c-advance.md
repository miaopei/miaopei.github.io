...
![函数调用流程](program-c-advance/函数调用流程.png)

栈的生长方向和内存存放方向：

![栈的生长方向和内存存放方向](program-c-advance/栈的生长方向和内存存放方向.png)
...
![二级指针](program-c-advance/二级指针.png)
...
- 对于非标准数据类型，比如结构体，要遵循一下对齐原则：
  ...
  - 结构体做为成员的对齐规则。如果一个结构体B里嵌套另一个结构体A,还是以最大成员类型的大小对齐，但是结构体A的起点为A内部最大成员的整数倍的地方。（`struct B`里存有`struct A`，A里有char，int，double等成员，那A应该从8的整数倍开始存储。），结构体A中的成员的对齐规则仍满足原则1、原则2。
...
![文件读写案例](program-c-advance/文件读写案例.png)
...
![链表-01](program-c-advance/链表-01.png)
...
![链表-02](program-c-advance/链表-02.png)

- 不带头链表：头结点不固定，根据实际需要变换头结点(如在原来头结点前插入新节点，然后，新节点重新作为链表的头结点)。

![链表-03](program-c-advance/链表-03.png)

**单向链表、双向链表、循环链表**

- 单向链表：

![链表-04](program-c-advance/链表-04.png)

- 双向链表：

![链表-05](program-c-advance/链表-05.png)

- 循环链表：

![链表-06](program-c-advance/链表-06.png)
...
![条件编译](program-c-advance/条件编译.png)
...
![递归函数](program-c-advance/递归函数.png)
...
![递归函数-01](program-c-advance/递归函数-01.png)
